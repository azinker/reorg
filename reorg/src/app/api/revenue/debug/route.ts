...
import { getRevenueDebugData, RevenueServiceError } from "@/lib/services/revenue";
...
function handleRevenueError(error: unknown, scope: string) {
  ...
  return NextResponse.json({ error: "Revenue debug request failed" }, { status: 500 });
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getRevenueDebugData(user, requestedPlatforms);
    ...
    return handleRevenueError(error, "[revenue/debug] GET failed");
  }
}
