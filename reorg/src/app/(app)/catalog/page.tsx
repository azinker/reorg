...
/**
 * All three of these fetchers accept an optional AbortSignal. The catalog
 ...
 * fetches. Passing a signal lets the catalog's cleanup effect cancel
...
async function fetchGridData(signal?: AbortSignal): Promise<GridPayload> {
  try {
    ...
    //     /catalog then immediately clicks the sidebar to /help-desk;
...
export default function CatalogPage() {
  ...
  useEffect(() => {
    ...
    // catalog mount. When the user navigates away (e.g. to /help-desk),
    ...
    async function refreshGridIfChanged(force = false) {
      ...
      try {
        ...
        if (gridData.source === "error" && sourceRef.current === "db") {
          console.warn("[catalog] Background grid refresh failed, keeping existing data:", gridData.error);
          return;
        }
      ...
      <div className="flex h-full min-h-0 min-w-0 items-center justify-center overflow-hidden">
        <div className="w-full max-w-md px-6">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground">Loading catalog data...</span>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between text-[11px] text-purple-300/80">
            <span>Connecting to live catalog data</span>
      ...
      {source === "error" && (
        <div className="border-b border-red-500/20 bg-red-500/5 px-4 py-2">
          <div className="text-xs text-red-300">
            <div className="font-semibold">Catalog connection issue</div>
            <div className="mt-0.5">
              The live catalog data could not be loaded, so reorG is not showing sample rows
              here anymore.
            </div>
