...
function buildColumns(_result: ForecastResult) {
...
const HEADER_NOTES: Record<string, string> = {
  "Order Qty":
    "The recommended order quantity from the forecast type you selected (Simple or Smart).",
  "Simple Estimate":
...
function addGuideSheet(workbook: ExcelJS.Workbook) {
  ...
  const columnGuide: Array<[string, string, string]> = [
    ...
    ["Simple Estimate", HEADER_NOTES["Simple Estimate"], "Example: 0.267/day × 165 days = 44 − 36 on hand = 8. No safety, no model."],
