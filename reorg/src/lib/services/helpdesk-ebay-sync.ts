 ...
 *   1. Cheap probe: GetMyMessages summary → check NewMessageCount + LastModifiedTime.
 *      Returns ~1KB. Run every cron tick.
 *   2. Headers pull: only fired when summary indicates new activity. Returns
 *      ~5–20KB depending on volume. Used to detect specific message IDs to
 *      hydrate.
 ...
 * HelpdeskSyncCheckpoint.backfillCursor — each tick advances by ~7 days
 * (eBay's StartTime window cap) until backfillDone=true.
 *
 * Threading: messages are linked into HelpdeskTickets by a `threadKey`. The
 * key prefers ItemID + buyer pair for buyer↔seller threads on a single item;
...
/** Maximum span (days) per GetMyMessages headers call — eBay caps at 7. */
...
/** Wall-clock budget in ms — used to bail out gracefully. */
...
// ─── Per-integration / per-folder logic ──────────────────────────────────────
...
async function pollIntegrationFolder(
  ...
  // ── Stage 2: pull headers for the appropriate window ────────────────────
  const now = new Date();
  ...
  if (needsBackfill) {
    ...
    if (windowEnd.getTime() <= horizonStart.getTime()) {
      // Already past the horizon — backfill is done.
  ...
  // ── Read-state reconciliation (inbound, opt-in) ──────────────────────────
  // If the eBay UI was used to mark messages read, mirror that into reorG so
  // the agent doesn't see the same red-dot twice. We only flow eBay→reorG when
  // a message is now read on eBay (one-directional). Going the other way
  // (eBay says unread → reorG marks unread) would constantly fight an agent
  ...
  // ── Stage 3: hydrate bodies in chunks ────────────────────────────────────
  const startedAt = Date.now();
  let chunksFetched = 0;
...
// ─── Reconciliation: header → ticket + message ───────────────────────────────
...
async function reconcileMessages(args: ReconcileArgs): Promise<ReconcileResult> {
  ...
  for (const body of args.bodies) {
    ...
    try {
      await db.helpdeskMessage.create({
        data: {
          ...
          // sender uploaded as text/html — a huge swath of buyer-facing
    ...
    // ── Auto-resolve: if this OUTBOUND message is newer than every inbound
    ...
    // to" case during initial backfill — the agent already replied on eBay,
      ...
      if (!newerInbound) {
        await db.helpdeskTicket.update({
          ...
          data: {
            ...
            resolvedAt: sentAt,
            // resolvedBy intentionally null — this was the eBay reply, not a
    ...
    // ── Filters: only evaluate against fresh INBOUND mail. Outbound replies
...
// ─── Body sniffing & order-number extraction ─────────────────────────────────
 ...
 *
 * The regex matches:
 *   `\d{2}-\d{5}-\d{5}` — the 5-3-5 segmented order number, or
 *   `\d{10,16}-\d{5,15}` — the legacy numeric ItemID-TxnID pairing.
 */
 ...
 * path — that would constantly fight an agent who already triaged in reorG.
 *
...
// ─── Backfill: ebayOrderNumber for tickets missing it ────────────────────────
 ...
 *   2. Match against AutoResponderSendLog (free, our own data — every
...
export async function backfillTicketOrderNumbers(
  ...
  for (const ticket of tickets) {
    ...
    // 1. Stored messages — re-run extraction in case the original sync
    ...
    // 2. Auto-responder log — when reorG sent the first message on this
