...
export const PAGE_TOUR_STEPS: Record<OnboardingPageKey, TourStep[]> = {
  ...
  sync: [
    ...
    { id: "sync-2", target: "sync-pull-only", title: "Safety guardrail: pull-only", body: "This reminder means this page only reads marketplace changes into reorG. No pushes happen here.\n\nExample: You can safely refresh data before a team training session without changing any live listing." },
    { id: "sync-3", target: "sync-actions", title: "Global sync actions", body: "Use Sync All for a broad refresh, or let automatic background checks keep stores updated on schedule.\n\nExample: After API outage recovery, press Sync All once to quickly repopulate stale data." },
    { id: "sync-4", target: "sync-scheduler", title: "Automatic background updates", body: "This scheduler panel is your heartbeat monitor: last check, jobs started, jobs running, and health headline.\n\nExample: If \"Auto updates on\" but last check is old, you know automation may be paused or failing." },
    { id: "sync-5", target: "sync-jobs", title: "Latest scheduled update per store", body: "Review each store’s most recent scheduled pull result, mode, and item counts. This is your quick proof of periodic health.\n\nExample: TT shows failed last run while Shopify is green, so you focus TT credentials first." },
    { id: "sync-6", target: "sync-freshness", title: "Store freshness diagnostics", body: "These cards show if each store is refreshing inside its expected window and whether webhooks are arriving.\n\nExample: Pull status looks okay but webhook status is quiet, so you inspect webhook subscriptions." },
    { id: "sync-7", target: "sync-upcoming", title: "What happens next", body: "This tells you what is running now, what is ready now, and what is waiting for the next interval.\n\nExample: A store marked \"ready\" means it will start on the next scheduler tick without manual action." },
    { id: "sync-8", target: "sync-webhooks", title: "Recent store change notices", body: "These are incoming marketplace events that trigger targeted refresh behavior.\n\nExample: A product update webhook appears here, then you confirm the SKU refreshed in dashboard shortly after." },
    { id: "sync-9", target: "sync-automation-events", title: "Why the system acted", body: "Automation events explain major decisions: start, skip duplicates, finish, or backoff.\n\nExample: Event says \"skipped duplicate\" so you know the system prevented wasted repeat work." },
    { id: "sync-10", target: "sync-stores", title: "Per-store sync cards", body: "Each store card has connection status, last sync details, cooldown/rate-limit context, and a Store-specific \"Sync Now\" button.\n\nExample: Only BigCommerce is stale, so you run sync on BC instead of all stores." },
    { id: "sync-11", target: "sync-stores", title: "Error handling on cards", body: "When a store sync fails, expanded details show row-level errors and recent webhook/schedule context so you can diagnose root cause faster.\n\nExample: Card shows repeated auth errors, so you reconnect token on Integrations before retrying." },
  ],
  integrations: [
  ...
  engineRoom: [
    ...
    { id: "er-2", target: "engine-summary", title: "Top summary cards", body: "These cards are your fast triage snapshot: active syncs, queued pushes, recent failures, scheduler status, and lock state.\n\nExample: Active syncs at 0 + stale data reports means scheduler may not be dispatching." },
    { id: "er-3", target: "engine-recent-errors", title: "Recent failures card", body: "This card shows failures in the last 7 days, not only current issues. You may see a failure here even if the store is healthy right now.\n\nExample: Yesterday's temporary outage still appears for history while today’s syncs are green." },
    { id: "er-4", target: "engine-write-lock", title: "Write lock status", body: "Write Lock controls whether pushes can write to marketplaces. Sync pulling can still happen while lock is on.\n\nExample: Keep lock ON during testing so no accidental production updates are sent." },
    { id: "er-5", target: "engine-scheduler", title: "Scheduler health panel", body: "This area explains automation timing, due counts, dispatch counts, and latest scheduler error if one exists.\n\nExample: Due is high but Dispatched is low, so you investigate rate limits or scheduler failures." },
    { id: "er-6", target: "engine-tabs", title: "Choose the right tab", body: "Tabs split concerns: Sync Jobs for pulls, Push Jobs/Queue for writes, Change Log for edits, and Raw Events for low-level traces.\n\nExample: Price changed unexpectedly? Start in Change Log, then trace related push job." },
    { id: "er-7", target: "engine-table", title: "Read status and timing", body: "In tab panels, review status, started/completed times, duration, and counts. Timing patterns reveal throttling or stuck workloads.\n\nExample: Jobs always fail near 2 minutes -> likely timeout boundary." },
    { id: "er-8", target: "engine-table", title: "Failure diagnosis workflow", body: "Use failure fields (category, summary, recommended action) to route issues to the right fix path quickly.\n\nExample: Auth category -> reconnect credentials; validation category -> correct bad SKU data." },
    { id: "er-9", target: "engine-table", title: "Operational handoff", body: "Engine Room records are ideal for handoff notes because they include IDs and timestamps teammates can verify.\n\nExample: Share push job ID + failure summary with the on-call teammate for immediate follow-up." },
  ],
  errors: [
